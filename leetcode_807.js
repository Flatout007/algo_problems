/* 
 * 807. Max Increase to Keep City Skyline [Medium]

There is a city composed of n x n blocks, where each block contains a 
single building shaped like a vertical square prism. You are given a 0-indexed n x n integer matrix grid where grid[r][c] represents 
the height of the building located in the block at row r and column c.

A city's skyline is the the outer contour formed by all the building when 
viewing the side of the city from a distance. The skyline from each cardinal
direction north, east, south, and west may be different.

We are allowed to increase the height of any number of buildings by any 
amount (the amount can be different per building). The height of a 0-height 
building can also be increased. 

However, increasing the height of a building should not affect the 
city's skyline from any cardinal direction.

Return the maximum total sum that the height of the buildings can be 
increased by without changing the city's skyline from any cardinal direction.

Example 1:
Input: grid = [[3,0,8,4],[2,4,5,7],[9,2,6,3],[0,3,1,0]]
Output: 35
Explanation: The building heights are shown in the center of the above image.
The skylines when viewed from each cardinal direction are drawn in red.
The grid after increasing the height of buildings without affecting skylines is:
gridNew = [ [8, 4, 8, 7],
            [7, 4, 7, 7],
            [9, 4, 8, 7],
            [3, 3, 3, 3] ]
*/
/**
 * @param {number[][]} grid
 * @return {number}
 */
 var maxIncreaseKeepingSkyline = function(grid) {
    /*
    1.TRAVERSE & sum each column or "building" from each view;
    SET them in a array of tuples "[building#, sum]";
    
    2.SORT each tuple by its second element, "sum";
    (This is used to compare original views)
    
    3.ROTATE grid by 90deg then increment each cell of the column or "building";
    CHECKING if the order of the incremented building is the same as the original views;
    If it is repeat step#3 else skip that building;
    */

    // DOES NOT WORK!!!!

    let g = grid;
    let n, e, s, w;
    
    // north
    n = fun2(g);
    n = s.sort((a,b) => a[1] - b[1]);
    
    // west
    g = fun1(g);
    w = fun2(g);
    w = s.sort((a,b) => a[1] - b[1]);
    
    // south
    g = fun1(g);
    s = fun2(g);
    s = s.sort((a,b) => a[1] - b[1]);
    
    // east
    g = fun1(g);
    e = fun2(g);
    e = s.sort((a,b) => a[1] - b[1]);
    
    
};

var fun2 = function(grid) {
    // sums columns. returns array of column sums
    
    let arr = [], sum = 0;
    
    for(let j = 0; j<grid[0].length; j++) {
        for(let i = 0; i<grid.length; i++) {
            sum += grid[i][j];
        }
        arr[j] = [j, sum];
        sum = 0;
    }
    
    return arr;
};

var fun1 = function(grid) {
    // rotate grid by 90, returns grid
    
    let g = grid;
    
    for(let i = 0; i<g.length; g++) {
        for(let j = i; j<g[0].length; j++) {
            if(i !== j) {
                let tmp = g[i][j];
                g[i][j] = g[j][i];
                g[j][i] = tmp;
            }
        }
    }
    
    for(let i = 0; i<g.length; i++) {
        g[i] = g[i].reverse();
    }
    
    return g;
};
