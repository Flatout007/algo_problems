...
to 2 * n - 1 (inclusive) should form the second row of the constructed 2D array, 
and so on.
 ...
 */
 var construct2DArray = function(original, m, n) {
    let mat = new Array(m), og = original, k = 0;
    let count = 0;
    ...
    // if elements in mat does not equal og.len then,
    // its impossible to put all elements in mat, so return [].
    if(count !== og.length) return [];
    else
