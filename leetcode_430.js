...
a previous pointer, and an additional child pointer. This child pointer may or may not point to a
separate doubly linked list, also containing these special nodes. These child lists may have one or more 
...
in a single-level, doubly linked list. Let curr be a node with a child list. 
 ...
 var flatten = function(head) {
    ...
    while(head.next) {                 
        if(head.child) { 
            ...
            flatten(childList);
            head.next.prev = findLastNode(childList);
            //findLastNode(childList).next = head.next;
...
var findLastNode = function(head) {
    let h = head;
